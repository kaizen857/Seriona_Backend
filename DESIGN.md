...
- 文件监听库首选 `efsw`，基础版本可以先实现启动增量扫描和手动刷新；运行时监听是首个正式版本必须具备的能力。
...
- Linux 侧使用 MPRIS 库实现 D-Bus 集成，优先考虑基于 `sdbus-c++` 的 MPRIS 封装或直接基于 `sdbus-c++` 适配 MPRIS v2。
...
后续实现时可以把音频播放模块抽象为 `AudioPlaybackService` 或同等接口。接口名称可以调整，但语义需要保持稳定：命令由 `mediaController` 调用，事件只投递到 `BackendEventSink`。
...
后续编写详细代码任务时，音频播放模块可以按以下顺序拆分，避免一次性实现整个播放器：
...
- 基础功能阶段可以先完成启动增量扫描和手动刷新；首个正式版本必须加入运行时监听。
...
文件监听库首选 `efsw`。它是 MIT 许可的 C++ 跨平台 watcher，支持 Linux inotify、Windows IOCP、macOS FSEvents/kqueue、递归监听和通用轮询回退，依赖少，生态中也有较多嵌入式使用案例。`wtr/watcher` 设计更现代、头文件集成更轻、测试较充分，可作为备选；但其生态和长期稳定性仍需观察。`libfswatch` 后端覆盖很全，但项目整体更偏命令行工具/库组合，许可和集成形态对本项目不如 `efsw` 直接。手动实现 `inotify` + `ReadDirectoryChangesW` 只作为最后备选，因为跨平台递归监听、重命名配对、溢出恢复、符号链接和网络文件系统差异会很快膨胀为独立子项目。
...
Linux 适配层使用 MPRIS/D-Bus。MPRIS 字段映射优先使用 `xesam:title`、`xesam:artist`、`xesam:album`、`mpris:artUrl`、`mpris:length`、`mpris:trackid` 和 `xesam:url`；实现库优先考虑基于 `sdbus-c++` 的 MPRIS 封装，必要时直接用 `sdbus-c++` 暴露 `org.mpris.MediaPlayer2` 和 `org.mpris.MediaPlayer2.Player`。

Windows 适配层以 SMTC 为目标。后续如果音频输出/播放集成方案能自动接入 SMTC，则复用自动集成；如果没有自动集成，则由本模块手动调用 SMTC，写入音乐元数据、封面缩略图、播放状态和时间线属性，并处理系统按钮事件。
...
4. 基础功能阶段可以先完成启动增量扫描和手动刷新；运行时监听是首个正式版本必须具备的能力。
...
2. 目前仍以确定项目设计文档为主，暂不进入构建系统、目录细分和具体依赖落地。
3. 音频输出和文件监听已有首选方向，后续实现前仍需要最小原型验证暂停设备释放、格式协商、监听溢出和大目录行为。
