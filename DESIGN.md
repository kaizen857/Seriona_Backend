...
- 对外消费：订阅 `PlayerStateSnapshot`/`LibraryStateSnapshot`/`EqualizerStateSnapshot`（生效配置 + 181 点增益曲线 + 单调代数）/`SpectrumSnapshot`（频谱：订阅即收当前驻留快照，随后经 `SpectrumUpdated` 事件增量推送 60 段电平）/领域通知即可构建新前端；`AudioPlaybackService` 与 `FileScannerService` 也可独立使用。
