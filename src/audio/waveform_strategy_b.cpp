...
// Strategy B batches are deliberately coarse: enough packets to amortize decoder setup,
// but small enough for Todo 9 to submit independent units without retaining the full file.
constexpr std::size_t kStrategyBPacketBatchSize = 250;

using StrategyBPacketBatch = std::vector<WaveformPacketPtr>;

[[nodiscard]] std::runtime_error strategyBDecodeError(std::string message, int ffmpegCode) {
  return std::runtime_error{std::move(message) + ": " + ffmpegErrorDetail(ffmpegCode)};
}

[[nodiscard]] std::runtime_error strategyBDecodeError(std::string message, std::string detail) {
...
[[nodiscard]] WaveformCodecParametersPtr copyCodecParameters(const AVCodecParameters& parameters) {
  WaveformCodecParametersPtr copied{avcodec_parameters_alloc()};
  if (!copied) {
    throw strategyBDecodeError("failed to allocate strategy B codec parameters", "avcodec_parameters_alloc returned null");
  }

  const int result = avcodec_parameters_copy(copied.get(), &parameters);
  if (result < 0) {
    throw strategyBDecodeError("failed to copy strategy B codec parameters", result);
  }
  return copied;
}

[[nodiscard]] WaveformCodecContextPtr openStrategyBDecoder(const AVCodecParameters& parameters,
                                                           AVRational streamTimeBase) {
  const AVCodec* decoder = avcodec_find_decoder(parameters.codec_id);
  if (decoder == nullptr) {
    throw strategyBDecodeError("strategy B audio decoder not found",
                               "codec id " + std::to_string(static_cast<int>(parameters.codec_id)));
  }

  WaveformCodecContextPtr codec{avcodec_alloc_context3(decoder)};
  if (!codec) {
    throw strategyBDecodeError("failed to allocate strategy B audio decoder", "avcodec_alloc_context3 returned null");
  }

  int result = avcodec_parameters_to_context(codec.get(), &parameters);
  if (result < 0) {
    throw strategyBDecodeError("failed to configure strategy B audio decoder", result);
  }

  codec->pkt_timebase = streamTimeBase;
  codec->thread_count = 1;
  result = avcodec_open2(codec.get(), decoder, nullptr);
  if (result < 0) {
    throw strategyBDecodeError("failed to open strategy B audio decoder", result);
  }
...
[[nodiscard]] WaveformKernelSampleFormat kernelSampleFormat(AVSampleFormat format) {
  ...
  throw strategyBDecodeError("unsupported strategy B waveform sample format",
...
[[nodiscard]] std::vector<const void*> kernelPlanes(const AVFrame& frame,
  ...
  if (frame.extended_data == nullptr) {
    throw strategyBDecodeError("strategy B decoded frame is missing sample data", "extended_data is null");
  }
  ...
  case WaveformKernelSampleLayout::Interleaved: {
    if (frame.extended_data[0] == nullptr) {
      throw strategyBDecodeError("strategy B decoded frame is missing interleaved samples", "extended_data[0] is null");
    }
  ...
  case WaveformKernelSampleLayout::Planar:
    for (int channel = 0; channel < channelCount; ++channel) {
      if (frame.extended_data[channel] == nullptr) {
        throw strategyBDecodeError("strategy B decoded frame is missing planar channel data",
  ...
  throw strategyBDecodeError("strategy B decoded frame has unknown sample layout", "unknown layout");
}
...
[[nodiscard]] WaveformKernelResult computeFrameSegment(const AVFrame& frame,
  ...
  if (channels <= 0) {
    throw strategyBDecodeError("strategy B decoded frame has invalid channel count", std::to_string(channels));
  }
  ...
  throw strategyBDecodeError("strategy B decoded frame has unsupported sample format", "unreachable format dispatch");
}

[[nodiscard]] std::int64_t packetTimelineTimestamp(const AVPacket& packet) {
...
[[nodiscard]] std::int64_t packetBaseSample(const StrategyBPacketBatch& packets,
                                            AVRational streamTimeBase,
                                            int sampleRate,
                                            std::int64_t fallbackSample) {
  for (const auto& packet : packets) {
    if (!packet) {
      continue;
    }
    const auto timestamp = packetTimelineTimestamp(*packet);
    if (timestamp != AV_NOPTS_VALUE) {
      return av_rescale_q(timestamp, streamTimeBase, AVRational{1, sampleRate});
    }
  }
...
[[nodiscard]] bool accumulateStrategyBFrameBars(const AVFrame& frame,
                                                int fallbackChannels,
  ...
  if (baseSample >= globalEndSample) {
    return false;
  }

  const auto frameEndSample = baseSample + frame.nb_samples;
  if (frameEndSample <= globalStartSample) {
    return true;
  }

  const auto processStartSample = std::max(baseSample, globalStartSample);
  const auto processEndSample = std::min(frameEndSample, globalEndSample);
  if (processStartSample >= processEndSample) {
    return processEndSample < globalEndSample;
  }
  ...
  while (consumedSamples < processSampleCount) {
    ...
    if (count <= 0 || count > std::numeric_limits<int>::max()) {
      throw strategyBDecodeError("strategy B sample span is invalid", std::to_string(count));
    }

    const auto frameOffset = firstFrameOffset + consumedSamples;
    if (frameOffset < 0 || frameOffset > std::numeric_limits<int>::max()) {
      throw strategyBDecodeError("strategy B frame offset is invalid", std::to_string(frameOffset));
    }
...
[[nodiscard]] std::vector<BarData> decodePacketBatchStrategyB(StrategyBPacketBatch packets,
  ...
  if (bars.empty() || packets.empty() || globalStartSample >= globalEndSample) {
    return bars;
  }

  auto decoder = openStrategyBDecoder(codecParameters, streamTimeBase);
  const int waveformSampleRate = sampleRate(*decoder);
  const int waveformChannels = channelCount(*decoder);
  if (waveformSampleRate <= 0) {
    throw strategyBDecodeError("strategy B audio stream has invalid sample rate", std::to_string(waveformSampleRate));
  }
  if (waveformChannels <= 0) {
    throw strategyBDecodeError("strategy B audio stream has invalid channel count", std::to_string(waveformChannels));
  }

  WaveformFramePtr frame{av_frame_alloc()};
  if (!frame) {
    throw strategyBDecodeError("failed to allocate strategy B waveform frame", "av_frame_alloc returned null");
  }

  const int decimation = waveformDecimationForSampleRate(waveformSampleRate);
  auto nextFallbackSample = packetBaseSample(packets, streamTimeBase, waveformSampleRate, globalStartSample);
  auto receiveFrames = [&]() {
    while (true) {
      ...
      if (receiveResult < 0) {
        throw strategyBDecodeError("failed to decode strategy B waveform frame", receiveResult);
      }

      const auto baseSample = frameBaseSample(*frame, nextFallbackSample, streamTimeBase, waveformSampleRate);
      const bool needsMore = accumulateStrategyBFrameBars(*frame,
                                                          waveformChannels,
                                                          baseSample,
                                                          globalStartSample,
                                                          globalEndSample,
  ...
  bool needsMore = true;
  for (const auto& packet : packets) {
    if (!packet || packet->size <= 0 || !needsMore) {
      continue;
    }
  ...
  if (needsMore) {
    ...
    if (drainResult < 0 && drainResult != AVERROR_EOF) {
      throw strategyBDecodeError("failed to drain strategy B waveform decoder", drainResult);
    }
...
std::vector<BarData> processPacketBatchStrategyB(const StrategyBPacketBatchRequest& request) {
  ...
  auto codecParameters = copyCodecParameters(*stream.codecpar);

  const int waveformSampleRate = sampleRate(*codecParameters);
  const int waveformChannels = channelCount(*codecParameters);
  if (waveformSampleRate <= 0) {
    throw strategyBDecodeError("strategy B audio stream has invalid sample rate", std::to_string(waveformSampleRate));
  }
  if (waveformChannels <= 0) {
  ...
  if (streamTimeBase.den == 0 || streamTimeBase.num == 0) {
    throw strategyBDecodeError("strategy B audio stream has invalid time base", "time_base numerator or denominator is zero");
  }

  const auto seekTimestamp = av_rescale_q_rnd(globalStartSample,
  ...
  if (seekResult < 0) {
    throw strategyBDecodeError("failed to seek strategy B waveform packet reader", seekResult);
  }

  const auto endTimestamp = av_rescale_q_rnd(globalEndSample,
                                            AVRational{1, waveformSampleRate},
                                            streamTimeBase,
                                            AV_ROUND_UP);
  WaveformPacketPtr packet{av_packet_alloc()};
  if (!packet) {
    throw strategyBDecodeError("failed to allocate strategy B waveform packet", "av_packet_alloc returned null");
  }

  StrategyBPacketBatch currentBatch;
  currentBatch.reserve(kStrategyBPacketBatchSize);
  ...
  auto flushBatch = [&]() {
    ...
    auto batch = std::make_shared<StrategyBPacketBatch>(std::move(currentBatch));
    currentBatch = StrategyBPacketBatch{};
    currentBatch.reserve(kStrategyBPacketBatchSize);
    tasks.push_back([batch,
      ...
      return decodePacketBatchStrategyB(std::move(*batch),
                                        codecParameters,
                                        streamTimeBase,
                                        barCount,
                                        globalStartSample,
  ...
  while (true) {
    ...
    if (readResult < 0) {
      throw strategyBDecodeError("failed to read strategy B waveform packet", readResult);
    }

    if (packet->stream_index == stream.index) {
      const auto timestamp = packetTimelineTimestamp(*packet);
      if (timestamp != AV_NOPTS_VALUE && timestamp > endTimestamp) {
      ...
      if (packet->size > 0) {
        currentBatch.push_back(clonePacket(*packet));
        if (currentBatch.size() >= kStrategyBPacketBatchSize) {
          flushBatch();
        }
      }
    }
  ...
  flushBatch();
  return scheduleWaveformBarTasks(request.barCount, std::move(tasks), request.config, "strategy B waveform thread pool");
}
