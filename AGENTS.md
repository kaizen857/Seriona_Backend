...
- Linux MPRIS 总线名获取与降级：`SdbusMprisBus::acquireBusName`（实现 `src/metadata/metadata_mpris_linux.cpp`，声明 `metadata_mpris_private.h`）先申请常规名 `org.mpris.MediaPlayer2.seriona`；被占用（sdbus-c++ 将 RequestName 的 EXISTS 结果码译为 `org.freedesktop.DBus.Error.FileExists`）时按 MPRIS 2.2 多实例约定回退 `org.mpris.MediaPlayer2.seriona.instance<pid>` 并记录到 `registeredName_`，两者皆不可得或其它 DBus 错误时告警并让 metadata 启动返回失败结果 `metadata.backend.mpris_unavailable`；`MediaController` 启动对 metadata `start()` 另有兜底 try/catch，仅告警不中断（MPRIS 是可选控制面，注册失败不得使控制器不可用）。回归测试：`seriona.metadata_mpris`（名占用回退/全占用降级）与 `seriona.control_controller`（metadata 启动抛异常仍可扫描）。
