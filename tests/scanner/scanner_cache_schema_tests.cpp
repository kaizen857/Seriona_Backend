...
[[nodiscard]] std::filesystem::path schemaPath() {
  return std::filesystem::path{__FILE__}.parent_path().parent_path().parent_path() / "src/scanner/cache/schema.sql";
}
...
[[nodiscard]] std::string sqliteObjectSql(sqlite3* db, const char* objectName) {
  sqlite3_stmt* statement = nullptr;
  REQUIRE(sqlite3_prepare_v2(db, "SELECT sql FROM sqlite_master WHERE name=?1;", -1, &statement, nullptr) == SQLITE_OK);
  REQUIRE(sqlite3_bind_text(statement, 1, objectName, -1, SQLITE_TRANSIENT) == SQLITE_OK);
  REQUIRE(sqlite3_step(statement) == SQLITE_ROW);
  const auto* sqlText = sqlite3_column_text(statement, 0);
  std::string sql{reinterpret_cast<const char*>(sqlText)};
  REQUIRE(sqlite3_finalize(statement) == SQLITE_OK);
  return sql;
}
...
TEST_CASE("sqlite scanner cache schema artifact validates against a temp database") {
  const auto tempDatabasePath = std::filesystem::temp_directory_path() / "seriona-scanner-schema-tests.sqlite";
  ...
  REQUIRE(std::filesystem::exists(artifactPath, readError));
  const auto sql = readTextFile(artifactPath);
  ...
  CHECK(hasObject(objects, "idx_locations_root_file_cue_identity", "index"));
  CHECK(hasObject(objects, "idx_lyrics_location", "index"));
  CHECK(hasObject(objects, "idx_errors_root", "index"));
  CHECK(sqliteObjectSql(db, "idx_locations_root_file_cue_identity") ==
        "CREATE UNIQUE INDEX idx_locations_root_file_cue_identity ON locations(root_path, file_path, COALESCE(cue_track_offset_ms, -1), COALESCE(cue_track_index, -1))");
  ...
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "cue_track_index"));
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "cue_track_duration_ms"));
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "cue_file_size_bytes"));
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "cue_file_mtime_ns"));
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "source_file_size_bytes"));
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "source_file_mtime_ns"));
  ...
  CHECK(hasColumn(sqliteColumnNames(db, "locations"), "external_lrc_hash"));
  ...
  CHECK(sqlite3_exec(db, "INSERT INTO locations(location_id, content_id, root_path, file_path, file_size_bytes, file_mtime_ns, source_file_path, cue_track_offset_ms, cue_track_index, cue_track_duration_ms, cue_file_size_bytes, cue_file_mtime_ns, source_file_size_bytes, source_file_mtime_ns, artwork_path, thumbnail_path, lyrics_source, external_lrc_path, external_lrc_mtime_ns, external_lrc_hash, discovered_at_ms, scanned_at_ms) VALUES('location-1', 'content-1', '/music', '/music/track.flac', 4096, 1000, '/music/source.flac', 5, 0, 180000, 4096, 1000, 8192, 3000, '/music/cover.jpg', '/music/thumb.jpg', 'embedded', '/music/track.lrc', 2000, 'lrc-hash-1', 10, 11);", nullptr, nullptr, nullptr) == SQLITE_OK);
  ...
  CHECK(sqlite3_exec(db, "INSERT INTO locations(location_id, content_id, root_path, file_path, file_size_bytes, file_mtime_ns, source_file_path, cue_track_offset_ms, cue_track_index, cue_track_duration_ms, cue_file_size_bytes, cue_file_mtime_ns, source_file_size_bytes, source_file_mtime_ns, artwork_path, thumbnail_path, lyrics_source, external_lrc_path, external_lrc_mtime_ns, external_lrc_hash, discovered_at_ms, scanned_at_ms) VALUES('location-2', 'content-1', '/music', '/music/track.flac', 4096, 1000, '/music/source.flac', 5, 1, 120000, 4096, 1000, 8192, 3000, '/music/cover.jpg', '/music/thumb.jpg', 'embedded', '/music/track.lrc', 2000, 'lrc-hash-2', 10, 11);", nullptr, nullptr, nullptr) == SQLITE_OK);
  CHECK(sqlite3_exec(db, "INSERT INTO locations(location_id, content_id, root_path, file_path, file_size_bytes, file_mtime_ns, source_file_path, cue_track_offset_ms, cue_track_index, cue_track_duration_ms, cue_file_size_bytes, cue_file_mtime_ns, source_file_size_bytes, source_file_mtime_ns, artwork_path, thumbnail_path, lyrics_source, external_lrc_path, external_lrc_mtime_ns, external_lrc_hash, discovered_at_ms, scanned_at_ms) VALUES('location-3', 'content-1', '/music', '/music/track.flac', 4096, 1000, '/music/source.flac', 5, 0, 180000, 4096, 1000, 8192, 3000, '/music/cover.jpg', '/music/thumb.jpg', 'embedded', '/music/track.lrc', 2000, 'lrc-hash-1', 10, 11);", nullptr, nullptr, nullptr) != SQLITE_OK);
  CHECK(sqlite3_exec(db, "INSERT INTO locations(location_id, content_id, root_path, file_path, file_size_bytes, file_mtime_ns, source_file_path, cue_track_offset_ms, cue_track_index, cue_track_duration_ms, cue_file_size_bytes, cue_file_mtime_ns, source_file_size_bytes, source_file_mtime_ns, artwork_path, thumbnail_path, lyrics_source, external_lrc_path, external_lrc_mtime_ns, external_lrc_hash, discovered_at_ms, scanned_at_ms) VALUES('location-4', NULL, '/music', '/music/other.flac', 4096, 1000, '/music/other.flac', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'embedded', NULL, NULL, NULL, 10, 11);", nullptr, nullptr, nullptr) != SQLITE_OK);
